...
import { VuvasTouchEvent } from '../../Node';

export type VuvasScrollEventType = 'start' | 'scroll' | 'end' | 'none';

export interface VuvasScrollEvent {
  type: VuvasScrollEventType;
  x: number;
  y: number;
  vx: number;
  vy: number;
...
export default class Scroller {
  ...
  constructor(private listener: (e: VuvasScrollEvent) => any) {}
  ...
  private _sign(e: VuvasTouchEvent) {
  ...
  private _check(e: VuvasTouchEvent) {
  ...
  emit(type: VuvasScrollEventType) {
    this.listener({
      type,
      x: this._x.offset,
      vx: this._x.velocity,
