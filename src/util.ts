...
export function setShadow(canvas: any, color: string, x: number, y: number, blur: number) {
  if (color) {
...
export function pushOpacity(canvas: any, opacity: number) {
...
export type RevasAdapter = {
  ...
  resetOffscreenCanvas?: (ctx: any, width: number, height: number) => any;
};
